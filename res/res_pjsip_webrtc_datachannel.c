 ...
 *
 * \brief WebRTC Datachannel handling
 */
...
static const char STR_APPLICATION[] = "application";

/*! \brief Address for webRTC datachannel */
static struct ast_sockaddr address;

/*! \brief webrtc datachannel information */
...
static struct ast_sip_session_supplement webrtc_datachannel_supplement = {
    .method = "INVITE",
...
static int ast_webrtc_datachannel_fd(const struct ast_webrtc_datachannel *datachannel)
{
    return datachannel->fd;
}

/*! \brief Destructor for T.38 state information */
static void ast_webrtc_datachannel_destroy(void *obj)
{
    ast_free(obj);
}
    ...
    int index, struct ast_stream *asterisk_stream)
{
    ...
    if (!session->endpoint->media.webrtc_datachannel_configuration.enabled) {
        ast_debug(3, "Declining; WebRTC Datachannel_configuration not enabled on session\n");
        return 0;
    }
    ...
    if ((ast_sockaddr_is_ipv6(addrs) && !session->endpoint->media.webrtc_datachannel_configuration.ipv6) ||
        (ast_sockaddr_is_ipv4(addrs) && session->endpoint->media.webrtc_datachannel_configuration.ipv6)) {
    ...
    struct pjmedia_sdp_session *sdp, const struct pjmedia_sdp_session *remote, struct ast_stream *stream)
{
    ...
    if (!session->endpoint->media.webrtc_datachannel_configuration.enabled) {
        ast_debug(3, "Not creating outgoing SDP stream: WebRTC Datachannel not enabled\n");
        return 1;
    } else
...
static struct ast_frame *media_session_webrtc_datachannel_read_callback(struct ast_sip_session *session, struct ast_sip_session_media *session_media)
{
    struct ast_frame *frame = NULL;

    if (!session_media->webrtc_datachannel) {
...
static int media_session_webrtc_datachannel_write_callback(struct ast_sip_session *session, struct ast_sip_session_media *session_media, struct ast_frame *frame)
{
    if (!session_media->webrtc_datachannel) {
        return 0;
    }
    ...
    const struct pjmedia_sdp_session *remote, int index, struct ast_stream *asterisk_stream)
{
    ...
    if (!session_media->webrtc_datachannel) {
        ast_debug(3, "Not applying negotiated SDP stream: no WebRTC Datachannel session\n");
        return 0;
    }
    ...
    ast_sip_session_media_set_write_callback(session, session_media, media_session_webrtc_datachannel_write_callback);
    ast_sip_session_media_add_read_callback(session, session_media, ast_webrtc_datachannel_fd(session_media->webrtc_datachannel),
        media_session_webrtc_datachannel_read_callback);
...
/*! \brief Function which destroys the Webrtc Datachannel instance when session ends */
static void stream_destroy(struct ast_sip_session_media *session_media)
{
    if (session_media->webrtc_datachannel) {
        ast_webrtc_datachannel_destroy(session_media->webrtc_datachannel);
    }
    session_media->webrtc_datachannel = NULL;
}

/*! \brief SDP handler for 'application' media stream */
static struct ast_sip_session_sdp_handler application_sdp_handler = {
	.id = STR_APPLICATION,
...
/*! \brief Unloads the SIP WebRTC Datachannel module from Asterisk */
static int unload_module(void)
{
    ast_sip_session_unregister_sdp_handler(&application_sdp_handler, STR_APPLICATION);
    ast_sip_session_unregister_supplement(&webrtc_datachannel_supplement);
...
static int load_module(void)
{
    ...
    ast_sip_session_register_supplement(&webrtc_datachannel_supplement);

	if (ast_sip_session_register_sdp_handler(&application_sdp_handler, STR_APPLICATION)) {
...
AST_MODULE_INFO(ASTERISK_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP WebRTC Datachannel Support",
